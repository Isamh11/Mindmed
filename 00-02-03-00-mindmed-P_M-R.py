...
########################################
    ...
    def save_pomodoro_informatom(self):
        try:
            mydb = con.connect(host="localhost", user="root", password="Isam2023", db = "MindMed")
            cursor = mydb.cursor()
            #employee_id =self.l111.text()
            ...
            date_of_adding_session=  datetime.date.today() 
            num_pomodoro_session= self.tb605.text()

         
            sql_insert_student = "insert into pomodoro (first_name,last_name,department , num_of_session,date_of_session) values(%s,%s,%s,%s,%s)"
            value = (first_name,last_name, department,num_pomodoro_session,date_of_adding_session)
            
            if(first_name!="" and last_name!="" ):
                cursor.execute(sql_insert_student ,value)
                mydb.commit()
                self.l600.setText("student information saved successfully")
                QMessageBox.information(self, "Mental health management system","student session added successfully!")
            else:
                QMessageBox.information(self,"Mental Health Management System", "Missing student session, Try again !")
                self.l600.setText("IMissing student session, Try again !")

            self.l600.setText("Pomodoro")
            self.tb602.setText("")
            self.tb603.setText("")
            ...
            self.l600.setText("Error occured in save student information" + e)
        finally:
            cursor.close()
            mydb.close()
